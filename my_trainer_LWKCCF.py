...
import models
from models.LWK_CCF.LWK_CCF import BCBLearner
...
class train_utils(object):
    def setup(self):
        ...
        if self.model_name != 'LWK_CCF':
            assert "Selected model is not involved"

        # step 1: 建立SubModule，多任务学习，每个学习任务基于二元的伪标签完成跨类滤波与分类
        if dataset_name in ['CQU_gear_dataset', 'SEU_bearing_dataset']:
            BCBLearner1 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner2 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner3 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner4 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner5 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            ...
            BCBLearner4 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner5 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner6 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner7 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner8 = BCBLearner(num_WKs=12, len_WKs=193, num_classes=2).to(self.device)
            BCBLearner1 = self.pre_train_BCBLearner(BCBLearner=BCBLearner1, fake_trainSet=self.dataloaders['fake_train1'])
            BCBLearner2 = self.pre_train_BCBLearner(BCBLearner=BCBLearner2, fake_trainSet=self.dataloaders['fake_train2'])
            BCBLearner3 = self.pre_train_BCBLearner(BCBLearner=BCBLearner3, fake_trainSet=self.dataloaders['fake_train3'])
            BCBLearner4 = self.pre_train_BCBLearner(BCBLearner=BCBLearner4, fake_trainSet=self.dataloaders['fake_train4'])
            ...
            self.model = getattr(models, self.model_name)(num_WKs=12, num_classes=Dataset.num_classes,
                                                          BCBLearner1=BCBLearner1, BCBLearner2=BCBLearner2,
                                                          BCBLearner3=BCBLearner3, BCBLearner4=BCBLearner4,
