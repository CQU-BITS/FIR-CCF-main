# Reference: S. Yan, H. Shao, J. Wang, X. Zheng, B. Liu, LiConvFormer: A lightweight fault diagnosis framework using
#            separable multiscale convolution and broadcast self-attention, Expert Syst. Appl., 237 (2024) 121338.
...
class LiConvFormer(nn.Module):
    def __init__(self, in_channels, num_classes, drop=0.1, dim=32):
        super(LiConvFormer, self).__init__()

        self.in_layer = nn.Sequential(
            nn.AvgPool1d(2, 2),
            ConvBNReLU(in_channels, dim, kernel_size=15, stride=2)
        )
        ...
        self.out_layer = nn.Linear(8*dim, num_classes)
...
if __name__=='__main__':
    model = LiConvFormer(in_channels=1, num_classes=10)
    model.eval()
    # print(model)
