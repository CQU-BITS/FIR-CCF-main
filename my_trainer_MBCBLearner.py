...
class train_utils(object):
    def setup(self):
        ...
        if self.model_name == 'M_BCBLearner':
            # step 1: 建立SubModule，多任务学习，每个学习任务基于二元的伪标签完成跨类滤波与分类
            self.model = getattr(models, self.model_name)(num_FIRs=12, len_FIRs=193, num_classes=Dataset.num_classes).to(self.device)
        else:
            assert "Selected model is not involved"

        # 定义 loss 评判准则和优化器, 只更新 global classifier 的参数
        self.criterion = nn.CrossEntropyLoss().to(self.device)
        wc_list = (param for name, param in self.model.named_parameters() if 'FIRFiltering.wc' in name)
        bw_list = (param for name, param in self.model.named_parameters() if 'FIRFiltering.bw' in name)
        others_list = (param for name, param in self.model.named_parameters() if 'FIRFiltering' not in name)
        self.optimizer = optim.Adam([{'params': wc_list, 'lr': 5e-3},
