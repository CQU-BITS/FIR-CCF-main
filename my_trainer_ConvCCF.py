...
import models
from models.Conv_CCF.Conv_CCF import BCBLearner
...
class train_utils(object):
    def setup(self):
        ...
        if dataset_name in ['CQU_gear_dataset', 'SEU_bearing_dataset']:
            ...
            self.dataloaders = {x: DataLoader(self.datasets[x],
                                ...
                                for x in ['fake_train1', 'fake_train2', 'fake_train3', 'fake_train4', 'fake_train5', 'real_train', 'real_test']}
        ...
        if dataset_name in ['CQU_gear_dataset', 'SEU_bearing_dataset']:
            BCBLearner1 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner2 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner3 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner4 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner5 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            ...
            self.model = getattr(models, self.model_name)(num_CKs=12, num_classes=Dataset.num_classes,
            ...
            BCBLearner4 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner5 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner6 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner7 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner8 = BCBLearner(num_CKs=12, len_CKs=3, num_classes=2).to(self.device)
            BCBLearner1 = self.pre_train_BCBLearner(BCBLearner=BCBLearner1, fake_trainSet=self.dataloaders['fake_train1'])
            BCBLearner2 = self.pre_train_BCBLearner(BCBLearner=BCBLearner2, fake_trainSet=self.dataloaders['fake_train2'])
            BCBLearner3 = self.pre_train_BCBLearner(BCBLearner=BCBLearner3, fake_trainSet=self.dataloaders['fake_train3'])
            BCBLearner4 = self.pre_train_BCBLearner(BCBLearner=BCBLearner4, fake_trainSet=self.dataloaders['fake_train4'])
    ...
    def pre_train_BCBLearner(self, BCBLearner, fake_trainSet):
        ...
        optimizer = optim.Adam(BCBLearner.parameters(), lr=1e-3)
        ...
        for epoch in range(300):
            ...
            print('{} epoch, BCBLearner 预训练Loss: {:.8f}, 预训练精度: {:.4%}  >> [{}/{}]'.format(
